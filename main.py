...
if (runQuasi1D):
    ...
    F_vec,x_throat,Mach_vec = areaFunc(Geom_Dat[:,0],Geom_Dat[:,1],X[3:-2],True)
...
for i in range(1,Nt+1):
    ...
    L0 = spatial_rhs(char_numerical_flux(q),q,dx) + q1d_rhs(F_vec,q[3:-2,:])
    q1[3:-2,:] = q[3:-2,:] + L0*dt
    
    q1 = update_ghost_pts(X,q1)
    L1 = spatial_rhs(char_numerical_flux(q1),q1,dx) + q1d_rhs(F_vec,q1[3:-2,:])
    q2[3:-2,:] = (3/4)*q[3:-2,:] + (1/4)*q1[3:-2,:] + (1/4)*L1*dt
    
    q2 = update_ghost_pts(X,q2)
    L2 = spatial_rhs(char_numerical_flux(q2),q2,dx) + q1d_rhs(F_vec,q2[3:-2,:])
    q[3:-2,:] = (1/3)*q[3:-2,:] + (2/3)*q2[3:-2,:]  + (2/3)*L2*dt    
